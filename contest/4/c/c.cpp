...
int main() {
  ...
  map<string, int> used_name_count;
  REP(i, N) {
    cin >> name[i];
    if (used_name_count.count(name[i])) {
      // used
      cout << name[i] + to_string(used_name_count[name[i]]) << endl;
      used_name_count[name[i]]++;
    } else {
      // not used
      OUT("OK");
      used_name_count.insert(make_pair(name[i], 1));
    }
  }
