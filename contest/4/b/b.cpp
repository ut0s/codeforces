...
int main() {
  ...
  REP(i, d) {
    int minTime, maxTime;
    cin >> minTime >> maxTime;
    schedule[i] = make_pair(minTime, maxTime);
    sum_min += minTime;
    sum_max += maxTime;
  }

  if (sum_min <= sunTime && sunTime <= sum_max) {
    OUT("YES");
    vector<int> ans(d);
    int diff = sunTime - sum_min;
    REP(i, d) {
      int minTime = schedule[i].first;
      int maxTime = schedule[i].second;
      ans[i]      = minTime;
      int gap     = maxTime - minTime;
      while (gap != 0 && diff != 0 && ans[i] != maxTime) {
        ans[i]++;
        diff--;
      }
      cout << ans[i] << " ";
